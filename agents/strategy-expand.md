...
- **Entity IDs out, never people.** You return the roster URI (IDs + the two classification columns) and counts. You never `entity_enrich`, never `resolve_and_enrich_rows`, never echo a resolved name / email / address, and never write contact data. Turning IDs back into identifiers or records — including the activate-stage identifier maximalism (each entity enriched back out to all its identifier types so the platform reaches more of them on its side) — is the **audience-activator**'s lane, downstream, behind the activate skill's confirmation.
