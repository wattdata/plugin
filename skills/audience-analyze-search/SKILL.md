...
Render the profiled pool per the render contract, grouped by role (defining / must-have / exclusion): each signal with name · what it means · reach (size) · actively-searching (intent) · match-to-brief (relevance) · concentration (rarity) — strongest first within each group, sizes human-rounded, the profiler's axes visible so "why is X above Y" is answerable from the render. The fenced working-set record rides beneath it as the durable carrier. Then end the turn at the pivot question.
