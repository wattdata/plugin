...
- **A status or list ask** — "what have I filed", "status of WATT-212", "anything changed lately", or a session-start note from the ticket-check hook asking for recent ticket changes → a read (step 6).
...
- **Am I up to date** → if the session-start update notice is in this conversation's context (it surfaces when a newer release is out — "you're on Y, X is available, here's what's changed, update at…"), answer from it and pass along its update link. **Don't fetch a version yourself for this** — that check owns the call. Otherwise: the top dated `## [x.y.z]` in the public changelog is the latest *released* version — name it and what's recent, and say you can't confirm their installed version this session unless they ask you to read it locally.
