...
Dispatch `audience-activator` with the confirmed platform, the confirmed ceiling, the location, the artifact's `workflow_id`, and the script's absolute path — plus the audience itself, in whichever form the input takes:
