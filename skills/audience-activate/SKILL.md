...
- **Tools absent, or a call comes back unauthenticated** → you don't have a live connection to run the export. Stop here, before any confirmation; don't retry blindly, don't loop the auth tools, and don't improvise a local file. Send the user to `/watt:quickstart` to get the connection fixed — it owns the connect path and the recovery docs. Then have them re-run once it's connected.
