...
# The contact-lane domains the Meta export materializes together — everything
# but the mobile-ad ID. ``maid`` is the heavy domain, so it is NOT pulled here:
# it rides its own lane and is merged back onto each person by entity_id, where
# write_meta places it raw in the ``madid`` column (and a device-ID-only person
# becomes a madid-only row). The two lanes reunite into one flat row, so the
# transform below is unchanged.
...
USAGE = """usage: meta.py [--help] [--list-identifiers] [--export-cap]
               [--uses-maid] [--maid-cap]
...
def main(argv):
    ...
    total, written = write_meta(rows, args["out_dir"])
