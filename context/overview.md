...
- **OAuth happens automatically.** The first Watt MCP tool call this session will pop a browser for Watt sign-in. Mention it before the first call so the user isn't surprised.
...
- **If the user types something without a slash command, read the intent.** Curiosity ("what's out there for X", "who are these people") → `/watt:explore`; build, profile, list, or export ("build me an audience of…", "I need 2M people who…", "match / expand / score my customer list", "find more like my customers", "how many / who's in my market", "an audience profile for my client", "export this to Meta / Google") → `/watt:audience`; a question about using Watt or whether the data exists, or wanting to report a problem ("what can you do", "how do I…", "do you have data on X", "something's broken", "I need a human") → `/watt:help`.

## Visual rendering

Every Watt skill's rich visual renders **inline in the conversation** through the host's visual tool — its setup guidance fetched silently once per session, both visual modules (*not* the side-panel artifact, *not* raw HTML pasted into a message) — one on every beat that has signals to show, and where the host returns the pick, the closing decision's options answer through the visual itself. **Every rule of the how lives once, in the plugin's `context/visuals.md`** — the pipeline, the render facts, the Watt palette, the decision wiring, the override of the host tool's styling defaults. It lands in context automatically right after the visual tool's setup call; if it hasn't by render time, read it before authoring the widget. The one fact local to this orientation: the shareable analyze **report** is the single visual that's a saved file (self-contained HTML, no JS, no PII), a deliverable the user keeps — not an inline decision visual.
